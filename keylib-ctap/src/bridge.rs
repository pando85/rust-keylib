...
///
/// Implements the `keylib_transport::CommandHandler` trait to allow
...
#[cfg(feature = "transport")]
impl<C: AuthenticatorCallbacks> keylib_transport::CommandHandler for TransportBridge<C> {
    fn handle_command(
        &mut self,
        cmd: keylib_transport::Cmd,
        data: &[u8],
    ) -> keylib_transport::Result<Vec<u8>> {
        match cmd {
            keylib_transport::Cmd::Cbor => {
            ...
            keylib_transport::Cmd::Msg => {
                // CTAP1/U2F message - not yet supported
                Err(keylib_transport::Error::Other(
            ...
            _ => {
                ...
                Err(keylib_transport::Error::InvalidCommand)
            }
        }
    }
}
...
mod tests {
    ...
    use keylib_transport::CommandHandler;
    ...
    fn test_cbor_command() {
        ...
        let result = bridge.handle_command(keylib_transport::Cmd::Cbor, &data);
        assert!(result.is_ok());
    ...
    fn test_msg_command_unsupported() {
        ...
        let data = vec![];
        let result = bridge.handle_command(keylib_transport::Cmd::Msg, &data);
