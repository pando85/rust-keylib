...
fn handle_set_pin<C: AuthenticatorCallbacks>(
    ...
    let valid = match protocol {
        1 => keylib_crypto::pin_protocol::v1::verify(&hmac_key, &new_pin_enc, &expected_mac),
        2 => keylib_crypto::pin_protocol::v2::verify(&hmac_key, &new_pin_enc, &expected_mac),
        _ => return Err(StatusCode::InvalidParameter),
    };
    ...
    let decrypted_pin = match protocol {
        1 => keylib_crypto::pin_protocol::v1::decrypt(&enc_key, &new_pin_enc)?,
        2 => keylib_crypto::pin_protocol::v2::decrypt(&enc_key, &new_pin_enc)?,
        _ => return Err(StatusCode::InvalidParameter),
    };
...
fn handle_change_pin<C: AuthenticatorCallbacks>(
    ...
    let valid = match protocol {
        1 => keylib_crypto::pin_protocol::v1::verify(&hmac_key, &verify_data, &expected_mac),
        2 => keylib_crypto::pin_protocol::v2::verify(&hmac_key, &verify_data, &expected_mac),
        _ => return Err(StatusCode::InvalidParameter),
    };
...
mod tests {
    ...
    fn test_set_pin_integration() {
        ...
        let platform_keypair = keylib_crypto::ecdh::KeyPair::generate().unwrap();
        let shared_secret = platform_keypair.shared_secret(&auth_public_key).unwrap();

        // Step 3: Derive keys
        let (enc_key, hmac_key) = keylib_crypto::pin_protocol::v1::derive_keys(&shared_secret);
