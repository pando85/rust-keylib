...
use keylib_transport::{ChannelManager, Cmd, CtapHidHandler, Message, Packet};
...
impl keylib_transport::CommandHandler for MockCommandHandler {
    fn handle_command(
        ...
        match cmd {
            ...
            Cmd::Msg => Err(keylib_transport::Error::Other(
                "MSG not supported".to_string(),
            )),
            _ => Err(keylib_transport::Error::InvalidCommand),
        }
    }
}
